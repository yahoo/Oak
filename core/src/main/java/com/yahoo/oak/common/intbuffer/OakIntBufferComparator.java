...
public class OakIntBufferComparator implements OakComparator<ByteBuffer> {
    ...
    public static int compare(final ByteBuffer buff1, final int pos1, final int size1,
        final ByteBuffer buff2, final int pos2, final int size2) {
        final int minSize = Math.min(size1, size2);

        for (int i = 0; i < minSize; i++) {
            final int offset = Integer.BYTES * i;
            final int i1 = buff1.getInt(pos1 + offset);
            final int i2 = buff2.getInt(pos2 + offset);

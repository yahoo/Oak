...
public class OakRKeyReference implements OakRBuffer {
    ...
    public void unsafeCopyBufferToIntArray(int srcPosition, int[] dstArray, int countInts) {
        UnsafeUtils.unsafeCopyBufferToIntArray(getTemporaryPerThreadByteBuffer(), srcPosition+keyPosition, dstArray, countInts);

    }

    private ByteBuffer getTemporaryPerThreadByteBuffer() {
        if (cachedBBreference != null) return cachedBBreference;
        ...
        if (memoryManager.isClosed())  throw new ConcurrentModificationException();
